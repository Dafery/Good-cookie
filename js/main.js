...
$(document).ready(function() {
  ...
  const questionBtns = $('.our-menu-question');
  ...
  $(window).on('resize', function() {
    ...
    if (windowW >= 767) {
      ...
      $('.menu__item-img').css('display', 'none');
    }
  });
    ...
    $(window).scroll(function(e) {
      if ($(window).width() >= 767) {
        if ($(window).scrollTop() >= 15) {
          ...
          $('.menu__item-img').css('display', 'block');
        }
      }
    });
  }
  ...
  $('.our-production-section__slider').slick({
  ...
  function toggleInputCaption(event) {
    const target = event.target;
    const item = $(target).prev();
    (target.value.length > 0) ? item.show(): item.hide();
  }

  questionBtns.on('click', function(e) {
    const item = $(e.target).closest('.our-menu-item').find('.our-menu-popup');
    (!item.is(':visible')) ? item.show(): item.hide();
  });

  formInputs.on('keyup', function(e) {
    toggleInputCaption(e);
  });

  formInputs.on('blur', function(e) {
    toggleInputCaption(e);
  });
